...
def test_analyze_merges_fileReviews_from_all_batches(fake_groq):
    ...
    # Each batch makes 8 concurrent Groq calls (Security, Performance, Style, Impact,
    # TestGeneration, Architecture, HistoricalBug, Synthesizer).
    # Total = 24 calls.
...
def test_analyze_single_batch_still_works(fake_groq):
    ...
    assert data["generatedReadme"] == "# Fake Readme"
    # 1 batch with 7 specialized agents + 1 synthesizer = 8 calls
...
def test_analyze_non_first_batch_failure_is_skipped_not_fatal(monkeypatch):
    ...
    async def flaky_call(**kwargs):
        ...
        if is_synthesizer:
            ...
            filenames = list(filenames)
            payload = {
                "fileReviews": {name: {"bugs": [], "security": [], "optimization": [], "styling": [], "impact": [], "test_findings": [], "architecture": [], "historical_bugs": []} for name in filenames}
            }
            if is_first_batch:
                payload["generatedReadme"] = "# Fake Readme"
                payload["mermaidDiagram"] = "graph TD\n  A[\"S\"] --> B[\"E\"]"
